...
public class UserEventQue {
    ...
    //单例模式使用UserEventQue
    //单例模式：只在类中创建一个私有静态对象，通过静态方法调用，可以保证这个程序中有且只有一个该对象
    ...
    public UserEvent nextUserArrive(EUserCategory category){
        ...
        //思考：保证无论什么情况下，都只会有一个用户加入队列，如果没有synchronized，当有两个用户同时加入，可能会导致用户覆盖。
        synchronized (UserEventQue.class){
            event.setSeq(SEQ++);
            events.add(event);
            System.out.println("一位"+category+"用户取号。");
        }
     ...
     * @return
     */
    public UserEvent dispatchUser() throws Exception {
            //使用工厂类动态加载算法实现类
            IUserSelector selector = UserSelectorFactory.getSelector();
            if(selector != null){
                //保证线程安全；TODO: 思考，这里为什么需要保证线程安全
                //思考：保证物无论什么情况下，都只会有一个用户被分派，如果没有synchronized，当有两个用户同时被分派，可能会导致一个用户进入两个柜台重复处理。
                if(events.size()>0) {
                    synchronized (UserEventQue.class) {
